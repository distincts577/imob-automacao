...
/**
 * Versões "genéricas" acima, usadas para guardar blobs de credenciais que
 * não são a config principal de um provider — ex.: a sessão do Baileys
 * (backend/src/services/baileysAuthStore.js), que precisa persistir no
 * banco (e não em disco) porque hosts como Render não garantem disco
 * persistente entre deploys/restart.
 */
async function saveEncryptedBlob(key, obj) {
  const encryptedConfig = encrypt(obj);
  return prisma.integration.upsert({
    where: { provider: key },
    update: { encryptedConfig },
    create: { provider: key, encryptedConfig },
  });
}

async function getEncryptedBlob(key) {
  const row = await prisma.integration.findUnique({ where: { provider: key } });
  if (!row) return null;
  return decrypt(row.encryptedConfig);
}

async function deleteEncryptedBlob(key) {
  await prisma.integration.deleteMany({ where: { provider: key } });
}
