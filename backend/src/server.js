...
// Se o provedor de WhatsApp configurado for o Baileys (QR code), tenta
// restaurar a sessão salva no banco ao subir o processo — evita precisar
// reescanear o QR a cada deploy/restart (comum em hosts como Render).
// Se não houver sessão salva ainda, connect() só entra em "qr_pending"
// silenciosamente, sem quebrar o boot do servidor.
getIntegrationConfig('whatsapp')
  .then((config) => {
    if (config?.provider === 'baileys') {
      return require('./services/providers/baileysAdapter').connect();
    }
  })
  .catch((err) => console.error('Falha ao restaurar sessão WhatsApp (Baileys):', err.message));
