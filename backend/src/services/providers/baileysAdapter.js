...
function normalizeToJid(toPhone) {
  ...
  // Fallback ingênuo — só usado se onWhatsApp() (ver resolveJid abaixo)
  // não conseguir confirmar o número por algum motivo.
  let digits = String(toPhone).replace(/\D/g, '');
  // Número brasileiro sem DDI (11 dígitos: DDD + 9 + número, ou 10 sem o
  // 9º dígito) — assume Brasil e prefixa 55. Sem isso o JID gerado não
  // corresponde a nenhuma conta real do WhatsApp.
...
/**
 * Confirma no próprio WhatsApp se o número existe e retorna o JID
 * canônico (às vezes difere do "chute" de normalizeToJid — ex.: números
 * brasileiros antigos sem o 9º dígito, ou o inverso). Sem essa checagem,
 * sock.sendMessage() para um número que não existe no WhatsApp costuma
 * resolver "com sucesso" sem entregar nada, e o sistema marca a
 * mensagem como enviada mesmo sem ter chegado a lugar nenhum.
 */
async function resolveJid(toPhone) {
  const guess = normalizeToJid(toPhone);
  const [result] = await sock.onWhatsApp(guess).catch(() => []);
...
async function sendMessage(_config, { toPhone, body }) {
  ...
  if (!jid) {
    throw new Error(
      `O número ${toPhone} não foi encontrado no WhatsApp (confira o DDI/DDD ou se o cliente realmente tem WhatsApp nesse número).`
    );
  }
