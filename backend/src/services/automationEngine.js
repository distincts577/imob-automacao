...
function isWithinQuietHours(now, quietStart, quietEnd) {
...
function isAllowedDay(now, allowedDaysOfWeek) {
  const isoDay = ((now.getDay() + 6) % 7) + 1; // 1=segunda ... 7=domingo
...
async function verifyBeforeSend(scheduledMessage) {
  const client = await prisma.client.findUnique({
    ...
    include: { deals: { where: { active: true }, take: 1 } },
  });
  ...
  if (rule) {
    ...
    if (!isAllowedDay(now, rule.allowedDaysOfWeek)) {
