...
4. O usuário admin é criado automaticamente no build (o `buildCommand` do `render.yaml` já roda `node prisma/seed.js`, que usa `upsert` — seguro rodar em todo deploy, não duplica nem reseta dados). Não precisa do Shell do Render (que é pago). Login inicial: `admin@imobiliaria.com` / `troque-esta-senha` (troque depois de entrar).
