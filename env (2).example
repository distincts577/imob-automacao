    ...
    <div>
      ...
      <div className="grid-cards" style={{ gridTemplateColumns: '1fr 1fr' }}>
        ...
        <div className="card">
          ...
          <p><strong>Próxima mensagem:</strong> {nextMessage ? new Date(nextMessage.scheduledFor).toLocaleString('pt-BR') : '—'}</p>
            ...
            <tbody>
              {client.stageHistory?.map((h) => (
                <tr key={h.id}>
                  <td>{h.fromStage || '—'}</td>
                  <td>{h.toStage}</td>
                  <td>{new Date(h.changedAt).toLocaleString('pt-BR')}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {client.messages?.map((m) => (
                <tr key={m.id}>
                  ...
                  <td>{new Date(m.scheduledFor).toLocaleString('pt-BR')}</td>
                  <td>{m.status}</td>
                </tr>
              ))}
            </tbody>
